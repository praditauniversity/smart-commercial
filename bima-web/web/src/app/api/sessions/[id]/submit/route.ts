...
export async function POST(
  ...
  try {
    ...
    // Build immutable snapshot of entire survey findings
    const nextVersionNumber =
      session.submissions.length > 0 ? session.submissions[0].versionNumber + 1 : 1;
    ...
    // Transaction: Create SubmissionVersion and update SurveySession status
    ...
    return NextResponse.json({ error: 'Gagal mengirim sesi survei.' }, { status: 500 });
  }
}
